...
<head>
	...
	<title>AgroTech - Ajout de Variété</title>
</head>
<body>
	<!-- SIDEBAR -->
	<section id="sidebar">
		...
		<main>
			<div class="head-title">
				<div class="left">
					...
					<ul class="breadcrumb">
						...
						<li>
							<a class="active" href="">Ajouter Variété</a>
						</li>
					</ul>
				</div>
			</div>
            @if (auth('agriculteur')->check())
                <div class="table-data">
                    <div class="order">
                        <div class="head">
                            <h3>Renseigner votre nouvelle variété de culture</h3>
                            <i class='bx bx-search' ></i>
                            <i class='bx bx-filter' ></i>
                        </div>
                        <table>
                            <form action="{{url('/variete/store')}}" method="post">
                                @csrf
                                <div class="container">
                                    <div class="form-box">
										...
										<label for="culture">Culture ou vous avez utilisé cette variété</label>
										<select name="culture_id" id="" class="field">
											@if(empty(auth('agriculteur')->user()->cultures))
												<div class="empty-one">Aucune Culture présente actuellement dans la base, veuillez ajouter une culture svp</div>
											@endif
